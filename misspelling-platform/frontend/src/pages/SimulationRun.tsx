...
export function SimulationRunPage() {
  ...
  const [topology, setTopology] = useState("watts_strogatz");
      ...
      <Space direction="vertical" size={18} style={{ width: "100%" }}>
        <Card bordered={false} className="algo-hero-card">
          <div className="algo-hero-head">
            ...
            <div className="algo-hero-side">
              <div className="algo-hero-note">
                ...
                <div className="algo-hero-note-value" style={{ fontSize: 22 }}>{String(summary?.topology ?? topology)}</div>
                <div className="algo-hero-note-copy">传播网络结构。</div>
              </div>
              <div className="algo-hero-note">
          ...
          <div className="algo-score-grid">
            ...
            <div className="algo-score-card">
              <div className="algo-score-label">Error RMSE</div>
              <div className="algo-score-value">
                {typeof summary?.error_rmse === "number" ? Number(summary.error_rmse).toExponential(2) : "--"}
              </div>
              <div className="algo-score-copy">错误拼写轨迹的偏差量级，更直观反映拟合精度。</div>
            </div>
            <div className="algo-score-card">
              <div className="algo-score-label">Phase Break</div>
              <div className="algo-score-value">{String(summary?.phase_break_year ?? "--")}</div>
              <div className="algo-score-copy">传播机制切换点。</div>
            </div>
            <div className="algo-score-card">
              <div className="algo-score-label">Best Scenario</div>
              <div className="algo-score-value" style={{ fontSize: 20 }}>{String(summary?.best_scenario ?? "--")}</div>
              <div className="algo-score-copy">干预后最终错误量改善最大的方案。</div>
            </div>
          </div>
        </Card>
            ...
            >
              <div className="algo-insight-list">
                <div className="algo-insight-card">
                  ...
                  <div className="algo-insight-copy">从正确拼写和全部错拼构造 `right / error / error_share / salience` 四条核心曲线。</div>
                </div>
                <div className="algo-insight-card">
                  ...
                  <div className="algo-insight-card">
                    ...
                    <div className="algo-insight-copy">
                      {variantBreakdown.length > 0
                        ? variantBreakdown
                            .slice(0, 3)
                            .map((item) => `${String(item.variant || "")} @ ${String(item.peak_year || "--")}`)
                            .join(" / ")
                        : "当前还没有可展示的错拼变体分布。"}
                    </div>
                  </div>
                  ...
                  {interventions.slice(0, 3).map((item) => (
                    <div className="algo-score-card" key={item.key} style={{ borderTop: `3px solid ${item.color}` }}>
                      ...
                      <div className="algo-score-copy">
                        {typeof item.start_year === "number" ? `从 ${item.start_year} 开始介入；` : ""}
                        最终错误量净减少值。
                      </div>
                    </div>
                  ))}
              ...
              <Card className="algo-table-card" bordered={false} bodyStyle={{ padding: 0, marginTop: 18 }}>
                <Table
                  ...
                  columns={[
                    ...
                    { title: "Observed Correct", dataIndex: "right_actual", render: (value: number) => value.toFixed(6) },
                    { title: "Observed Error", dataIndex: "error_actual", render: (value: number) => value.toFixed(6) },
                    { title: "Simulated Correct", dataIndex: "right_simulated", render: (value: number) => value.toFixed(6) },
                    { title: "Simulated Error", dataIndex: "error_simulated", render: (value: number) => value.toFixed(6) },
                    { title: "Observed Share", dataIndex: "error_share_actual", render: (value: number) => value.toFixed(4) },
                    { title: "Simulated Share", dataIndex: "error_share_simulated", render: (value: number) => value.toFixed(4) },
                  ]}
                />
              </Card>
            </Card>

...
from ..db.core import DATABASE_URL, get_engine
...
def _is_mysql_database() -> bool:
    url = str(DATABASE_URL or "").strip().lower()
    return url.startswith("mysql")


def ensure_project_analytics_schema() -> None:
    if not _is_mysql_database():
        return
    with get_engine().begin() as conn:
        conn.execute(text(PROJECT_COHORTS_SQL))
        conn.execute(text(PROJECT_TERM_MEMBERSHIPS_SQL))
        conn.execute(text(BACKFILL_DEFAULT_COHORT_SQL))
        conn.execute(text(BACKFILL_CATEGORY_COHORTS_SQL))
        conn.execute(text(BACKFILL_MEMBERSHIPS_SQL))

...
def Hermite(xdata,ydata):
    ...
    a = (f_x0_x1-f_x0_x1_x2*np.diff(xdata)[0])/(-np.diff(xdata)[0] * np.diff(xdata)[1])
    # f_x0:第一个点
    factor = np.array([f_x0, f_x0_x1, f_x0_x1_x2, a])
    # 计算第k阶基函数
    # 计算插值函数
    def px(factor, x, xdata):
        ...
        for i in range(len(factor)):
            ...
            if i < len(factor)-1:
                wk = wk*(x-xdata[i])
        return w@factor
    plot_x = np.linspace(xdata[0], xdata[-1], 12*(len(xdata)-1))
    plot_y = px(factor, plot_x, xdata)
    return plot_y[0:12]  # 插值1-12月的数据
...
def getNgrams(query, corpus, startYear, endYear, smoothing):
	...
	for index in range(len(termsSearched)):
		...
		# time = [i for i in range(startYear,endYear+1)]
		# y = [float(time) for time in timeseries[index].split(",")]
		# month = []
		# for i in range(len(time) - 2):
		# 	month_x0 = Hermite(time[i:i + 3], y[i:i + 3])
		# 	month.append(month_x0)
		# termsToTimeseries[termsSearched[index]] = month
...
def reOrganizeDataByYear(data, startYear, endYear):
	...
	# for i in range((endYear - startYear - 1)*12):
